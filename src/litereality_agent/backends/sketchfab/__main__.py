"""CLI:  python -m litereality_agent.backends.sketchfab {search,get,whoami}

  python -m litereality_agent.backends.sketchfab whoami
  python -m litereality_agent.backends.sketchfab search "office chair" -n 8 --max-faces 200000
  python -m litereality_agent.backends.sketchfab get <uid|url> --out out/objects --name my_chair
...
def _client() -> SketchfabClient:
    try:
        from litereality_agent.models.config import load_env  # repo .env (non-overriding); shell env wins
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="python -m litereality_agent.backends.sketchfab", description="Sketchfab fetch tool")
