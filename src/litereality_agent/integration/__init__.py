"""integration — the scene-definition tool for LiteReality-Agent.
...
def export_scene(scan: str, out_root: "Path | None" = None) -> "Path | None":
    ...
    from litereality_agent.integration import export_room
...
def compile_room(
    ...
    *,
    ...
    cmd = [sys.executable, "-m", "litereality_agent.integration.compile.build_from_room", "--room", str(room_dir)]
    if out_dir:
        cmd += ["--out", str(out_dir)]
    if regenerate:
