...
Given that folder — and nothing else — stage 2 can rebuild the whole environment::
    ...
    ...                                             # litereality_agent.realism_authoring.scene.config now resolves

From a shell, `python -m litereality_agent.integration.manifest env <dir>` prints those exports for
...
def require(hint: Path | str | None = None) -> ScenePackage:
    ...
    raise SystemExit(
        ...
        "  A package is written by `uv run -m litereality_agent scene_init <scan>`; an output tree from before packages existed "
        "can be upgraded in place with `uv run -m litereality_agent scene --adopt <scan>` (nothing is rebuilt)."
    )
...
def bootstrap(argv: list[str] | None = None, *, hint: Path | str | None = None) -> ScenePackage | None:
    ...
    `litereality_agent.realism_authoring.scene.config`, which resolves every harness path the moment it is imported).
...
# ── the shell interface ──────────────────────────────────────────────────────
# `python -m litereality_agent.integration.manifest <cmd> [<dir>]`
#
