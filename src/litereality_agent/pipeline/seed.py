...
def run(context: RunContext, options: dict) -> StageResult:
    from litereality_agent.pipeline import paths
    from litereality_agent.pipeline.artifacts import ensure_stage_links, finalize
    from litereality_agent.scene.builder import build_preview, export_initial_scene
    ...
    if options.get("preview", True):
        try:
            preview = build_preview(Path(room))
            if preview:
