...
Run (from repo root):
    python -m litereality_agent.pipeline.stages.reconstruct.classify.classify_complexity                       # all scans
    python -m litereality_agent.pipeline.stages.reconstruct.classify.classify_complexity --scan tea_room
"""
...
from litereality_agent.pipeline.stages.ingest.references import (
    classify_backend as gemini_classify,  # LR_IMAGE_PROVIDER=openai → OpenAI VLM
)
...
def classify_for_scan(scan: str, *, model: str | None = None, include_chairs: bool = True) -> dict:
    ...
    model = model or gemini_classify.DEFAULT_CLASSIFY_MODEL
    ...
    for _scan, kind, name, ref, meta in items:
        ...
        try:
            decision = gemini_classify.classify_image(ref, prompt, ROUTE_SCHEMA, model=model)
...
def main() -> int:
    ...
    ap.add_argument("--model", default=gemini_classify.DEFAULT_CLASSIFY_MODEL)
    args = ap.parse_args()
