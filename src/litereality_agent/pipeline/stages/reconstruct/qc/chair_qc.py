...
ones by **re-generating the Nano-Banana reference image** (a fresh, cleaner image —
...
standalone. Needs Gemini (image regen) + the TRELLIS GPU env (re-generation).
"""
...
from litereality_agent.pipeline.stages.ingest.references import chair_clusters
...
def _normalized_ref(scan: str, cid: str) -> Path:
    return config.chair_clusters_root() / scan / "nano_banana_normalized_1024" / f"{cid}.png"
    ...
    *,
    ...
    for glb in chairs:
        ...
        for attempt in range(1, max_attempts + 1):
            # 1. REGENERATE the nano-banana reference (force -> a fresh, different image)
            chair_clusters.run_gemini_for_cluster(scan, cluster, chair_root, model, force=True)
...
def main() -> int:
    ...
    ap.add_argument("--model", default=None, help="Gemini model for the image regen.")
