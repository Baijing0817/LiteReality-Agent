...
Needs a compiled `Room.glb` (build the room first) and `python-fcl`.
    ...
    python -m litereality_agent.pipeline.stages.quality.collision --room <dir>          # dry run: print the plan
    python -m litereality_agent.pipeline.stages.quality.collision --room <dir> --apply  # write the nudges into Room.py
"""
...
from litereality_agent.scene.quality import collision_mesh as sc
from litereality_agent.scene.quality.fix import (
    FREE_STANDING,
    MAX_NUDGE,
    _anchored,
    _obb,
...
from litereality_agent.scene.quality.room import _extract_shell
