...
the model pass; run the linter first/after if you want the arithmetic checks too.)
    ...
    LITEREALITY_BLENDER=<dir> .venv/bin/python authoring/qc_pass.py \
...
from litereality_agent.pipeline.stages.author.impl import (  # noqa: E402
    CAPABILITY_TOOLS,
...
try:
    from litereality_agent.services import tracing  # noqa: E402
...
async def run(room: Path, surface_ref: Path, scan: Path, refroot: Path, model: str, max_turns: int,
    ...
    from litereality_agent.services.tracing.narrate import ToolNarrator, hint_for
