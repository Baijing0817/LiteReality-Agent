...
uv run litereality stage ingest office-elliott
...
uv run litereality run office-elliott --through reconstruct

# just neural reconstruction of the trellis-route objects (needs the GPU env)
uv run litereality stage reconstruct run/office-elliott

# several scans; --scan takes a scans_uploaded name OR an absolute raw path
uv run litereality stage ingest office-elliott
uv run litereality stage ingest tea_room

# a raw folder elsewhere, with an explicit scan name
uv run litereality stage ingest /abs/path/to/raw_scan
```
