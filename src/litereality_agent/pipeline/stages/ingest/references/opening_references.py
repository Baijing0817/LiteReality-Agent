"""Stage 3b — door/window (opening) clean Nano Banana references.
...
weighted evidence sheet to Gemini with a door/window-specific prompt.
...
GroundingDINO. Writes:
    ...
    opening_refs/<scan>/<Opening>/{gemini_prompt.txt,reference_meta.json}
...
def generate_for_scan(
    ...
    *,
    ...
    for opening in openings:
        ...
        status, error, gemini_meta = "no_view", None, None
        crop_paths = []
        crops_meta = []
        if selected:
            ...
            for cand in selected:
                ...
                # opening is presented front-on to Gemini (matches object_references).
            ...
            if crops_only:
                ...
                (out_dir / "gemini_prompt.txt").write_text(prompt, encoding="utf-8")
                if skip_gemini:
                    status = "skipped_gemini"
                else:
                    try:
                        gemini_meta = gemini_image.nano_banana(
                            sheet,
                            prompt,
                            raw,
                            model=model,
                        ...
                        gemini_image.normalize_generation_image(raw, normalized)
                        status = "ok"
        ...
        if error:
            record["error"] = error
        if gemini_meta:
            record["gemini_meta"] = gemini_meta
        if selected:
...
def main() -> int:
    ...
    ap.add_argument("--model", default=config.DEFAULT_GEMINI_MODEL)
    ...
    print(f"\nopenings: {n_ok}/{n_tot} nano-banana ok -> {out_root}")
    return 0
