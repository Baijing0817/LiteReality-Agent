"""Stage 4 — group repeated chairs, then one Nano Banana reference per group.
...
and generates ONE clean Nano Banana reference per cluster. Writes:
    ...
    <output_root>/<scan>/nano_banana/<ChairCluster>.png
    <output_root>/<scan>/nano_banana_normalized_1024/<ChairCluster>.png
...
from . import image_backend as gemini_image  # LR_IMAGE_PROVIDER=openai → gpt-image-1, else Gemini
...
def make_cluster_generation_sheet(image_paths: list[Path], output: Path) -> None:
    """Primary chair view plus a small ordered support strip for Nano Banana."""
...
def cluster_scan(scan: str, output_root: Path) -> dict:
    ...
    for idx, group in enumerate(groups):
        ...
        evidence = []
        # Small ordered evidence set — too many crops make Nano Banana treat the
...
def run_gemini_for_cluster(
    ...
    out = output_root / scan / "nano_banana" / f"{cluster['cluster_id']}.png"
    if out.exists() and not force:
        cluster["nano_banana_render"] = str(out.relative_to(output_root))
        return
    ...
    prompt_path = output_root / scan / "nano_banana_prompts" / f"{cluster['cluster_id']}.txt"
    prompt_path.parent.mkdir(parents=True, exist_ok=True)
    prompt_path.write_text(prompt, encoding="utf-8")
    cluster["nano_banana_prompt"] = str(prompt_path.relative_to(output_root))
    ...
    try:
        gemini_image.nano_banana(
            ...
            model=model,
            force=force,
            response_dir=output_root / scan / "nano_banana" / "_responses",
        )
        cluster["nano_banana_render"] = str(out.relative_to(output_root))
        normalized = (
            output_root / scan / "nano_banana_normalized_1024" / f"{cluster['cluster_id']}.png"
        )
        gemini_image.normalize_generation_image(out, normalized)
        cluster["nano_banana_normalized_1024"] = str(normalized.relative_to(output_root))
        cluster["nano_banana_status"] = "ok"
    except Exception as exc:  # noqa: BLE001
        cluster["nano_banana_status"] = "error"
        cluster["nano_banana_error"] = str(exc)
    ...
    *,
    ...
    if not skip_gemini:
        for cluster in result["clusters"]:
            print(f"  [nano-banana] {cluster['cluster_id']} {cluster['members']}", flush=True)
            run_gemini_for_cluster(scan, cluster, output_root, model, force_gemini)
