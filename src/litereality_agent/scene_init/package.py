...
reason `./run.sh --scene <dir>` (or a bare `./run.sh` from inside one) can start authoring with no
scan name, no `$LR_SCANS_DIR`, and no per-stage path flags.

What it writes is described by :mod:`litereality_agent.integration.manifest` (layer 0, stdlib only); everything
...
def scene_root(scan: str) -> Path:
    ...
    from litereality_agent.scene_init.object_init import config as oi_config
...
def _capture_source(scan: str) -> Path:
    ...
    if candidate.is_dir():
        return candidate
    from litereality_agent.integration import config as sb_config
