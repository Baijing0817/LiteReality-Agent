"""Stage 3 — per-object crops -> one clean Nano Banana reference per object.
...
prompts Gemini ``gemini-2.5-flash-image`` for a clean object-only render, and
...
:mod:`object_init.chair_clusters` (they need grouping first). Writes:
    ...
    <output_root>/<scan>/<Object>/{gemini_prompt.txt,reference_meta.json}
...
def make_skip_gemini_reference(source: Path, destination: Path, canvas_size: int = 1024) -> None:
...
def generate_for_scan(
    ...
    *,
    ...
    for obj_dir in object_dirs(scan, only, include_openings, include_chairs):
        ...
        gemini_meta = None
        if skip_gemini:
            make_skip_gemini_reference(input_sheet, normalized)
        else:
            try:
