"""OpenAI vision classifier — a drop-in twin of :func:`gemini_classify.classify_image`.

Same signature; returns a parsed JSON decision. Uses an OpenAI vision model
(``$LR_OPENAI_VLM_MODEL``, default ``gpt-4o``) with JSON-object output. Selected by
``LR_IMAGE_PROVIDER=openai`` through :mod:`classify_backend`. The response_schema is
...
def classify_image(
    ...
    *,
    ...
    if not key:
        raise RuntimeError("OpenAI classify needs $OPENAI_API_KEY (LR_IMAGE_PROVIDER=openai).")
    # The callers pass the Gemini model name; ignore it and use the OpenAI VLM.
