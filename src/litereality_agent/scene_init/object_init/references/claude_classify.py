"""Claude vision classifier — a self-contained twin of :func:`gemini_classify.classify_image`.

Runs on the **logged-in Claude** via ``claude_agent_sdk`` (Read-only, one-shot), so it needs
**no API key**. Selected by ``LR_CLASSIFY_PROVIDER=claude`` (the default under the "no Gemini"
policy) through :mod:`classify_backend`.

Deliberately does NOT import the harness (``litereality_agent.realism_authoring.scene.config`` requires ``$LITEREALITY_SCAN``);
