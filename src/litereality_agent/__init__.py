...
authoring phases, ``agent`` owns extensible agent behavior, ``scene`` owns portable room behavior,
``models`` owns inference, and ``runtimes`` owns execution transports. The supported entrypoint is
``uv run litereality``.
