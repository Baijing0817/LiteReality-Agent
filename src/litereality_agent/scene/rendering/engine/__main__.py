"""CLI for the image-render-tool — four modes.
  ...
  python -m litereality_agent.scene.rendering.engine scene  --room R --frames 10,20        # all objects
  python -m litereality_agent.scene.rendering.engine wall   --room R --frames 10,20        # walls only
  python -m litereality_agent.scene.rendering.engine wall-focus --room R --frames 10 --wall Wall2   # ONE wall
  python -m litereality_agent.scene.rendering.engine object --room R --frames 10 --object Table0

  # wall reference (ortho render | stitch + wall-focused ref frames):
  python -m litereality_agent.scene.rendering.engine ref    --room R --walls Wall0,Wall2
"""
