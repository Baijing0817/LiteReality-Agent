"""Export a compiled room to a SINGLE self-contained Three.js HTML file.
...
The panels beyond the model itself (QC violations, the run trace, real-vs-render pairs) are
gathered by whoever knows where those artifacts live — see
`pipeline/realism_authoring/publish/viewer.py`, which collects them and calls in here.
  ...
  · QC       — deterministic geometry violations (no model involved).
  · COMPARE  — real-vs-render pairs at the capture's own ARKit poses, embedded as JPEG.
  · TRACE    — the run's own event log as a readable timeline with per-stage durations.
"""
...
def _compress_glb(glb: Path) -> Path:
    ...
    variable unset, every published viewer silently embedded an UNCOMPRESSED glb."""
    import subprocess
    import tempfile
...
def export_html(glb: str | Path, out: str | Path, label: str | None = None, compress: bool = True,
    ...
    qc, trace, pairs = qc or {}, trace or [], pairs or []
