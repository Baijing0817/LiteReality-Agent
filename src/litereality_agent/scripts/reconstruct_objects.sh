...
#
#   src/litereality_agent/scripts/reconstruct_objects.sh <scan> [<scan> ...]   # reconstruct these scenes (sequential)
#   src/litereality_agent/scripts/reconstruct_objects.sh --force <scan>        # rebuild even existing GLBs
...
for a in "$@"; do
  case "$a" in
    --force) FORCE="--force-reconstruct" ;;
    *) ARGS+=("$a") ;;
  esac
done
[ ${#ARGS[@]} -gt 0 ] || { echo "usage: src/litereality_agent/scripts/reconstruct_objects.sh [--force] <scan> [<scan> ...]"; exit 2; }
...
for S in "${ARGS[@]}"; do
  ...
  t0=$(date +%s)
  if $PY -m litereality_agent.scene_init.object_init.run --scan "$S" \
        --classify --reconstruct --procedural --build-openings \
        --concurrency "$CONCURRENCY" $FORCE >"$log" 2>&1; then
    dt=$(( $(date +%s) - t0 ))
