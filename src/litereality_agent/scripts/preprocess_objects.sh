...
# reconstruction can be decided/run later.  (Full seed init is still src/litereality_agent/scripts/init_batch.sh <scan>.)
#
# The worklist defaults to every scan directory under $LR_SCANS_DIR; pass scan names to narrow it.
#
#   src/litereality_agent/scripts/preprocess_objects.sh                 # preprocess every scan not done yet
#   src/litereality_agent/scripts/preprocess_objects.sh --force         # redo even scenes already grouped
#   src/litereality_agent/scripts/preprocess_objects.sh <scan> [<scan>] # just these
#
...
# (src/litereality_agent/scene_init/object_init/config.py::chair_clusters_root). Keep this root in sync with that config /
...
[ ${#SCENES[@]} -gt 0 ] || {
  ...
  echo "usage: src/litereality_agent/scripts/preprocess_objects.sh [--force] [<scan> ...]"
  exit 2
}
...
for S in "${SCENES[@]}"; do
  ...
  if [ -d "$R" ]; then
    ...
    if [ "$nd" != "$ni" ] || [ "$nd" != "$nk" ] || [ "$nd" != "$ne" ]; then
      ...
      rm -rf "$R"; FORCE_EXTRACT="--force-extract"
    fi
  fi
  ...
  if $PY -m litereality_agent.scene_init.object_init.run --scan "$S" --skip-gemini $FORCE_EXTRACT >"$log" 2>&1; then
    dt=$(( $(date +%s) - t0 ))
