...
#
#   src/litereality_agent/scripts/generate_objects.sh                 # generate for every scan
#   src/litereality_agent/scripts/generate_objects.sh <scan> [<scan>] # just these
#   src/litereality_agent/scripts/generate_objects.sh --force-images  # regenerate even existing images
#
...
[ ${#SCENES[@]} -gt 0 ] || {
  ...
  echo "usage: src/litereality_agent/scripts/generate_objects.sh [--force-images] [<scan> ...]"
  exit 2
}
EXTRA=""; [ "$FORCE_IMAGES" = 1 ] && EXTRA="--force-gemini"
...
for S in "${SCENES[@]}"; do
  ...
  if $PY -m litereality_agent.scene_init.object_init.run --scan "$S" $EXTRA >"$log" 2>&1; then
    dt=$(( $(date +%s) - t0 ))
