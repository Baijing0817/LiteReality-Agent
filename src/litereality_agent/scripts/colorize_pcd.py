...
in which it is actually visible (in-image, in front, depth-consistent, high-confidence).
  ...
  uv run python scripts/colorize_pcd.py [--conf 2] [--tol 0.06] [<scan> ...]
"""
...
def colorize(scan: str, conf_min: int, tol: float) -> str | None:
    ...
    if not os.path.exists(pcd_path):
        print(f"  ✗ {scan}: no pointcloud.pcd"); return None
    pcd = o3d.io.read_point_cloud(pcd_path)
    P = np.asarray(pcd.points)                       # N x 3 world
    if not len(P):
        print(f"  ✗ {scan}: empty pcd"); return None
    Ph = np.concatenate([P, np.ones((len(P), 1))], axis=1).T   # 4 x N
    csum = np.zeros((len(P), 3)); cnt = np.zeros(len(P))
    ...
    for i in ids:
        ...
        z = pc[2]
        front = z > 1e-6
        u = np.full(len(P), -1.0); v = np.full(len(P), -1.0)
