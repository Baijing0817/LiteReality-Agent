...
def _sync_stale_objects(room_dir) -> list[str]:
    ...
    if not proc.is_dir():
        return []
    try:
        from litereality_agent.services.rendering.engine.compose import (
            _config_for,
            _scan_from_room,
        )
...
async def render_target(scene_path, target: str, frames: list[int] | None, n: int = 4) -> dict:
    ...
    from litereality_agent.services.tools.compile.tool import (
        CompileInvocation,
        CompileParams,
        compile_is_fresh,
    )
    ...
    selection = None
    if not frames:
        from litereality_agent.services.tools.select_views.tool import (
    ...
    from litereality_agent.services.tools.render import engine as irt  # lazy: Blender
