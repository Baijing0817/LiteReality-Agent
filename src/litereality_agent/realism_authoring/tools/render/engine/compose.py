...
CLI:  python -m litereality_agent.realism_authoring.tools.render.engine <frames|surface> --room <room dir> [...]
API:  from litereality_agent.realism_authoring.tools.render.engine import render_frames, render_surface
"""
...
def _config_for(scan: str):
    ...
    from litereality_agent.realism_authoring.scene import config
...
# `run/` is the one stage root. It used to be two -- run.sh symlinked
...
def _scan_from_room(room_dir: Path) -> str:
    ...
    Falls back to `$LITEREALITY_SCAN`, which run.sh exports for every stage, so a room dir outside
...
def _run_blender_frames(room_dir: Path, spec: str, config, renders: Path) -> None:
    ...
    try:
        subprocess.run(
            [
                str(config.BLENDER), "-b", "--python", str(_HERE / "_blender_frames.py"), "--",
...
def render_wall_reference(
    ...
    *,
    ...
    from litereality_agent.realism_authoring.scene import surface_compare
    ...
    if with_refs:
        from litereality_agent.realism_authoring.scene import wall_refs
