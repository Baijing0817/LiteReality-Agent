...
HARNESS = Path(__file__).resolve().parent  # src/litereality_agent/realism_authoring/scene
AGENT_DIR = REPO_ROOT  # back-compat alias
ROOT = REPO_ROOT  # back-compat alias
...
STAGES_DIR = HARNESS.parent / "prompts" / "stages"
SDK = PACKAGE_ROOT / "realism_authoring" / "views"
...
#
# An already-exported environment WINS: a discovered package only fills gaps, so run.sh's
# behaviour is byte-for-byte what it was. Only an explicit --scene/$LR_SCENE overrides.
try:
    from litereality_agent.integration import manifest as _manifest

    SCENE = _manifest.bootstrap()
except Exception:  # noqa: BLE001 — the package layer is optional; env-only still works
    SCENE = None

SCAN = os.environ.get("LITEREALITY_SCAN", "").strip()
if not SCAN:
    raise SystemExit(
        "no scan context — set $LITEREALITY_SCAN, or point --scene / $LR_SCENE at a scene "
        "package (the folder holding scene.json that `uv run -m litereality_agent scene_init` writes), or run from inside one."
    )


def _output_root() -> Path:
    env = os.environ.get("LITEREALITY_OUTPUT") or os.environ.get("OBJECT_INIT_OUTPUT")
    return Path(env).expanduser().resolve() if env else (ROOT / "run")
...
os.environ.setdefault("SB_VIEW_OUT", str(HARNESS_OUT))
os.environ.setdefault("SB_SCAN_DIR", str(SCAN_DIR))
...
# initialise the live paths at the baseline room so dry-run / standalone verify work
# before the runner sets an iteration.
set_room(ROOM_INIT / "room", ROOM_INIT / "room_preview")
...
GLB_AGENT = PACKAGE_ROOT / "backends" / "procedural" / "articulated-glb-agent"
...
RECOLOR_TOOL = PACKAGE_ROOT / "integration" / "compile" / "recolor.py"
...
def _find_python() -> str:
    """A python with numpy + Pillow (select/annotate/metrics — NOT Blender)."""
    for c in (
        os.environ.get("HARNESS_PY"),
        ...
        try:
            ...
            subprocess.run([c, "-c", "import numpy, PIL"], check=True, capture_output=True)
            return str(c)
        except Exception:
            continue
    raise SystemExit("no python with numpy+PIL found — set $HARNESS_PY")
...
# VLM CRITIC backend = Gemini (fast one-shot vision grade via REST).
GEMINI_KEY_FILE = ROOT / ".key"
GEMINI_MODEL = os.environ.get("HARNESS_GEMINI_MODEL", "gemini-2.5-pro")


def gemini_api_key():
    """Gemini key from env, else the .key file (bare key or NAME=value / quoted)."""
    k = os.environ.get("GEMINI_API_KEY") or os.environ.get("GOOGLE_API_KEY")
    if k:
        return k.strip()
    if GEMINI_KEY_FILE.is_file():
        txt = GEMINI_KEY_FILE.read_text().strip()
        if "=" in txt:
            txt = txt.split("=", 1)[1]
