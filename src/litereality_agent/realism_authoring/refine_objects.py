...
Neural (TRELLIS) objects are baked meshes and are not refinable this way, so they are skipped.
    ...
    LITEREALITY_BLENDER=<blender dir> .venv/bin/python authoring/refine_objects.py --scene <scene dir>

`--scene` is the scene package `uv run -m litereality_agent scene_init` wrote; it supplies the room, the references and the
...
from litereality_agent.realism_authoring.narrate import hint_for

# Blender execs this by path, so it must be absolute: the CWD of a stage is not the repo.
OBJVIEW = str(PACKAGE_ROOT / "realism_authoring" / "objview.py")
...
def _montage(views_dir: Path, targets: list, out: Path, open_views: Path | None = None):
    ...
    for i, (img, label) in enumerate(panels):
        ...
        sheet.paste(b, (x, 0)); sheet.paste(img, (x, 35))
...
async def refine_one(name: str, room: Path, refroot: Path, scan: Path, blender: str, model: str, max_turns: int, counter: dict):
    ...
    t0 = time.monotonic(); calls = 0; counts: dict = {}; summary = ""; cost = None
            ...
            async with _SEM:
                async for m in query(prompt=prompt, options=options):
                    ...
                    if isinstance(m, ResultMessage):
                        summary = m.result or ""; cost = getattr(m, "total_cost_usd", None)
            last_err = None
...
def _procedural_objects(room: Path) -> list[str]:
    ...
    This is what run.sh computes for `REFINE_OBJECTS`; deriving it here too is what lets the stage
    run off a scene package alone. The module-level `OBJECTS` list is a last-resort fallback for a
