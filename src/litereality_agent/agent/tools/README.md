...
|---|---|
| [`../config.py`](../config.py) | harness paths and knobs — four tools plus the pipeline's `evidence.py` |
| [`../scan.py`](../scan.py) | `scan_from_room` / `config_for`; `tests/test_scan_inference.py` pins that exactly one copy exists |
