...
# is `uv run litereality run <scan>`; this wrapper only batches initialization.
#
# The worklist defaults to every scan directory under $LR_SCANS_DIR; pass scan names to narrow it.
#
#   scripts/pipeline/init_batch.sh                 # init every scan that isn't done yet
#   scripts/pipeline/init_batch.sh --force         # re-init even scenes that already have room_init
#   scripts/pipeline/init_batch.sh <scan> [<scan>] # init just these
#
...
[ ${#SCENES[@]} -gt 0 ] || {
  ...
  echo "usage: scripts/pipeline/init_batch.sh [--force] [<scan> ...]"
  exit 2
}
...
for S in "${SCENES[@]}"; do
  ...
  t0=$(date +%s)
  if uv run litereality run "$S" --through seed >"$log" 2>&1; then
    dt=$(( $(date +%s) - t0 ))
