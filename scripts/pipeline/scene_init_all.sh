...
#
#   scripts/pipeline/scene_init_all.sh                 # every scan
#   scripts/pipeline/scene_init_all.sh <scan> [...]    # just these
#
...
[ ${#SCENES[@]} -gt 0 ] || {
  ...
  echo "usage: scripts/pipeline/scene_init_all.sh [<scan> ...]"
  exit 2
}
...
for S in "${SCENES[@]}"; do
  ...
  if uv run litereality stage seed "$S" --force >"$log" 2>&1; then
