...
#
#   scripts/pipeline/reconstruct_all.sh                 # every scan, 5-wide
#   scripts/pipeline/reconstruct_all.sh <scan> [...]    # just these
#   LR_RECON_PARALLEL=3 scripts/pipeline/reconstruct_all.sh
#   scripts/pipeline/reconstruct_all.sh --force         # rebuild even existing GLBs
#
...
[ ${#SCENES[@]} -gt 0 ] || {
  ...
  echo "usage: [LR_RECON_PARALLEL=n] scripts/pipeline/reconstruct_all.sh [--force] [<scan> ...]"
  exit 2
}
...
for S in "${SCENES[@]}"; do
  ...
  echo "▶ launching $S"
  scripts/pipeline/reconstruct_objects.sh $FORCE "$S" > "run/_recon_${S}.log" 2>&1 &
done
wait
echo
